...
let s_or_p = rlSync.question(
  'Enter "s" to compute the sum, or "p" to compute the product:'
); //Sum or Product

function sumPrevious(num) {
  //Computes sum
  if (num === 0) return num;
  return num + sumPrevious(num - 1);
}

function multPrevious(num) {
  //Computes product
  if (num === 1) return num;
...
if (s_or_p === "s") {
  //Determines and executes appropriate action
  ...
  console.log(
    `The product of the integers between 1 and ${userNum} is ${result}.`
  );
} else {
