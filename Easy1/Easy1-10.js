...
function multisum(num) { //Computes sum of numbers previous, divisible by 3 or 5
  if (num === 0) return num; //Stopping condition

  if ((num % 3 === 0) || ((num % 5) === 0)) { //If a number is divisible by 3 or 5, returns that number and adds it to total
    return num + multisum(num - 1);
  }

  return multisum(num - 1); //If a number is not divisible by 3 or 5 checks next number
}
